...
const INPUT = (err?: string) =>
  `w-full rounded-xl border ${err ? 'border-red-500/40 bg-red-500/5' : 'border-white/[0.08] bg-[#161616]'} px-4 py-3 text-sm text-white placeholder-white/20 outline-none focus:border-[#F5A623]/40 transition-colors`;
...
export default function RegisterPage() {
  ...
  return (
    <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center px-4 py-10" style={{ background: '#0A0A0A' }}>
      <div className="w-full max-w-md">

        {/* Logo */}
        <div className="mb-6 text-center">
          <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-xl text-sm font-black border" style={{ background: 'linear-gradient(135deg,#0778BD,#0560A0)', color: '#F5A623', borderColor: 'rgba(245,166,35,0.2)' }}>CB</div>
          <h1 className="text-2xl font-black text-white">Create account</h1>
          <p className="mt-1 text-sm" style={{ color: '#666666' }}>Join Castle Bet and claim your welcome bonus</p>
        </div>
          ...
          {([1, 2, 3] as Step[]).map((s, i) => (
            <div key={s} className="flex flex-1 items-center gap-1">
              <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-black transition-colors"
                style={{ background: s <= step ? '#F5A623' : '#161616', color: s <= step ? '#0A0E14' : '#666666' }}>
                {s < step ? '✓' : s}
              </div>
              <p className="hidden text-[10px] font-semibold sm:block" style={{ color: s === step ? '#F5A623' : '#666666' }}>{stepLabels[i]}</p>
              {s < 3 && <div className="h-px flex-1 transition-colors" style={{ background: s < step ? '#F5A623' : 'rgba(255,255,255,0.07)' }} />}
            </div>
          ))}
        </div>

        <div className="rounded-2xl border border-white/[0.07] p-6" style={{ background: '#111111' }}>
          <form onSubmit={handleSubmit}>
            ...
            {step === 1 && (
              <div className="space-y-4">
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-bold" style={{ color: '#666666' }}>Password</label>
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-bold" style={{ color: '#666666' }}>Confirm password</label>
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-bold" style={{ color: '#666666' }}>Currency</label>
            ...
            {step === 2 && (
              <div className="space-y-4">
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-bold" style={{ color: '#666666' }}>Date of birth</label>
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-bold" style={{ color: '#666666' }}>Phone (optional)</label>
            ...
            {step === 3 && (
              <div className="space-y-4">
                <div className="rounded-xl p-4 space-y-2" style={{ background: '#161616' }}>
                  <p className="text-xs font-black uppercase tracking-wider mb-3" style={{ color: '#666666' }}>Account summary</p>
                ...
                <label className="flex cursor-pointer gap-3">
                  ...
                  <span className="text-xs" style={{ color: '#888888' }}>I&rsquo;d like to receive bonus offers by email</span>
                </label>
                <div className="flex gap-3">
                  <button type="button" onClick={back} className="flex-1 rounded-xl border border-white/10 py-3 text-sm font-semibold text-white/50 hover:bg-white/5">Back</button>
