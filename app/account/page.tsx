...
const INPUT = 'w-full rounded-xl border border-white/[0.08] bg-[#161616] px-4 py-3 text-sm text-white placeholder-white/20 outline-none focus:border-[#F5A623]/40 transition-colors';
...
export default function AccountPage() {
  ...
  return (
    <div className="pb-20 md:pb-6" style={{ background: '#0A0A0A', minHeight: '100vh' }}>
      ...
      <div className="px-4 py-5 border-b border-white/[0.06]" style={{ background: '#111111' }}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl text-sm font-black border" style={{ background: 'linear-gradient(135deg,#0560A0,#0778BD)', color: '#F5A623', borderColor: 'rgba(245,166,35,0.25)' }}>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                <span className="rounded-full px-2 py-0.5 text-[9px] font-black" style={{ background: 'rgba(7,120,189,0.25)', color: '#1A9FE0' }}>✓ Verified</span>
              </div>
            </div>
          </div>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2">
          {[
            ...
            <div key={s.label} className="rounded-xl p-3 text-center border border-white/[0.06]" style={{ background: '#161616' }}>
              <p className="text-[9px] font-bold uppercase tracking-wide mb-1" style={{ color: '#666666' }}>{s.label}</p>
      ...
      {/* Tabs */}
      <div className="flex border-b border-white/[0.06]" style={{ background: '#111111' }}>
        {TABS.map((t) => (
          <button key={t.value} onClick={() => setTab(t.value)}
            className="flex-1 py-3 text-xs font-bold transition-colors"
            style={{ color: tab === t.value ? '#F5A623' : '#666666', borderBottom: tab === t.value ? '2px solid #F5A623' : '2px solid transparent' }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-5 space-y-4">
        ...
        {tab === 'profile' && (
          <>
            <div className="rounded-2xl border border-white/[0.06] p-5 space-y-4" style={{ background: '#111111' }}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="mb-1.5 block text-xs font-bold" style={{ color: '#666666' }}>First name</label>
                  <input defaultValue={user.firstName} className={INPUT} />
                </div>
                <div>
                  <label className="mb-1.5 block text-xs font-bold" style={{ color: '#666666' }}>Last name</label>
                  <input defaultValue={user.lastName} className={INPUT} />
                </div>
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-bold" style={{ color: '#666666' }}>Email address</label>
                <input defaultValue={user.email} type="email" className={INPUT} />
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-bold" style={{ color: '#666666' }}>Phone number</label>
              ...
              <div className="flex justify-end">
                <button onClick={handleSave} className="rounded-xl px-6 py-2.5 text-sm font-black text-[#0A0E14] transition-colors"
                  style={{ background: saved ? '#1A9FE0' : '#F5A623' }}>
            ...
            {/* KYC */}
            <div className="rounded-2xl border border-white/[0.06] p-5" style={{ background: '#111111' }}>
              <p className="text-sm font-black text-white mb-3">Identity verification (KYC)</p>
              {/* TODO: Backend — GET /api/user/kyc for real status */}
              <div className="flex items-start gap-3 rounded-xl px-4 py-3 border" style={{ background: 'rgba(7,120,189,0.1)', borderColor: 'rgba(45,122,80,0.3)' }}>
                <span style={{ color: '#1A9FE0' }}>✓</span>
                <div>
                  <p className="text-sm font-semibold" style={{ color: '#1A9FE0' }}>Account fully verified</p>
                  <p className="text-xs mt-0.5" style={{ color: '#666666' }}>Your identity has been confirmed. No further action needed.</p>
                </div>
              </div>
            </div>
          </>
          ...
          <>
            <div className="rounded-2xl border border-white/[0.06] p-5 space-y-4" style={{ background: '#111111' }}>
              ...
              {['Current password', 'New password', 'Confirm new password'].map((l) => (
                <div key={l}>
                  <label className="mb-1.5 block text-xs font-bold" style={{ color: '#666666' }}>{l}</label>
            ...
            <div className="rounded-2xl border border-white/[0.06] p-5 space-y-3" style={{ background: '#111111' }}>
              <p className="text-sm font-black text-white">Two-factor authentication</p>
              <p className="text-sm" style={{ color: '#666666' }}>Add an extra layer of security to your account.</p>
              {/* TODO: Backend — POST /api/auth/2fa/setup */}
              <div className="flex items-center justify-between rounded-xl border border-white/[0.06] px-4 py-3" style={{ background: '#161616' }}>
                <div>
                  <p className="text-sm font-semibold text-white">Authenticator app</p>
                  <p className="text-xs" style={{ color: '#666666' }}>Not enabled</p>
                </div>
        ...
        {tab === 'responsible' && (
          <>
            <div className="rounded-2xl border border-white/[0.06] p-5 space-y-4" style={{ background: '#111111' }}>
              ...
              <p className="text-sm" style={{ color: '#666666' }}>Limits reduce immediately. Increases take 24 hours.</p>
              {/* TODO: Backend — PATCH /api/user/limits { daily, weekly, monthly } */}
              {['Daily limit', 'Weekly limit', 'Monthly limit'].map((l) => (
                <div key={l}>
                  <label className="mb-1.5 block text-xs font-bold" style={{ color: '#666666' }}>{l}</label>
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-xs font-bold" style={{ color: '#666666' }}>€</span>
            ...
            <div className="rounded-2xl border border-red-500/20 p-5 space-y-3" style={{ background: 'rgba(232,77,28,0.04)' }}>
              ...
              <p className="text-sm" style={{ color: '#888888' }}>Exclude yourself from Castle Bet for a set period if gambling is becoming a problem.</p>
              ...
              <p className="text-xs" style={{ color: '#666666' }}>Need help? <a href="https://www.begambleaware.org" target="_blank" rel="noreferrer" style={{ color: '#F5A623' }}>BeGambleAware.org</a></p>
            </div>
          </>
        )}
      </div>
