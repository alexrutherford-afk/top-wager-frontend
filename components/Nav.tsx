  ...
  return (
    <>
        ...
        >
          ...
          <Link href="/" className="flex items-center gap-2">
            <div
              ...
              style={{
                background: 'linear-gradient(135deg, #0778BD, #0560A0)',
                color: '#F5A623',
                borderColor: 'rgba(245,166,35,0.2)',
                boxShadow: '0 3px 10px rgba(7,120,189,0.5)',
              }}
            >
              CB
            </div>
            <div className="leading-none">
              <span className="block text-xs font-black tracking-widest" style={{ color: '#F5A623' }}>CASTLE</span>
              <span className="block text-xs font-black tracking-widest text-white">BET</span>
              <span className="block text-[8px] font-semibold tracking-widest uppercase" style={{ color: '#666666' }}>Casino</span>
            </div>
          </Link>
          ...
          {isLoggedIn && user ? (
            <div className="flex items-center gap-2">
              <Link
                ...
                style={{ background: 'linear-gradient(135deg, #0778BD, #0560A0)', boxShadow: '0 3px 10px rgba(7,120,189,0.4)' }}
              >
                + Deposit
              </Link>
              <Link href="/account" className="relative">
                <div
                  ...
                  style={{
                    background: 'linear-gradient(135deg, #0560A0, #0778BD)',
                    color: '#F5A623',
                    borderColor: 'rgba(245,166,35,0.3)',
                    boxShadow: '0 0 10px rgba(7,120,189,0.4)',
                  }}
                >
                  {user.firstName[0]}{user.lastName[0]}
                </div>
                <span className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-[#E84D1C] border-2 border-[#111111]" />
              </Link>
            </div>
          ) : (
            <div className="flex items-center gap-2">
        ...
        {isLoggedIn && user && (
          ...
          >
            <div className="flex flex-col px-3 py-1.5 border-r border-white/[0.07]">
              <span className="text-[8px] font-bold uppercase tracking-wider" style={{ color: '#666666' }}>💰 Main</span>
              <span className="text-sm font-black text-white">€{user.balance.cash.toLocaleString('en-GB', { minimumFractionDigits: 2 })}</span>
            </div>
            <div className="flex flex-col px-3 py-1.5 border-r border-white/[0.07]">
              <span className="text-[8px] font-bold uppercase tracking-wider" style={{ color: '#666666' }}>🎁 Bonus</span>
              ...
              >
                ...
                {tab.badge && (
                  <span
                    ...
                    style={{ background: tab.badgeOrange ? '#E84D1C' : '#0778BD', color: 'white' }}
                  >
                    {tab.badge}
                  </span>
                )}
      ...
      <nav
        ...
        style={{ background: '#111111' }}
      >
        <Link href="/" className={`flex flex-col items-center gap-0.5 px-3 py-1 ${pathname === '/' ? 'text-[#F5A623]' : 'text-[#666666]'}`}>
          <span className="text-[19px]">🏠</span>
          <span className="text-[8.5px] font-semibold">Home</span>
        </Link>
        <Link href="/?cat=slots" className="flex flex-col items-center gap-0.5 px-3 py-1 text-[#666666]">
          <span className="text-[19px]">🎰</span>
        ...
        <div className="flex flex-col items-center gap-0.5 -mt-4">
          <Link
            href="/"
            className="w-12 h-12 rounded-full flex items-center justify-center text-xs font-black border-[3px] border-[#111111]"
            style={{
              background: 'linear-gradient(135deg, #0778BD, #0560A0)',
              color: '#F5A623',
              boxShadow: '0 4px 16px rgba(7,120,189,0.6)',
            }}
          >
            CB
          </Link>
          <span className="text-[8.5px] font-bold" style={{ color: '#1A9FE0' }}>Play</span>
        </div>
        <Link
          href="/wallet"
          className={`flex flex-col items-center gap-0.5 px-3 py-1 ${isLoggedIn ? 'text-[#666666]' : 'text-[#666666] opacity-40 pointer-events-none'}`}
        >
          <span className="text-[19px]">💳</span>
